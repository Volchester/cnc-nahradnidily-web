...
- [x] **Google Search Console — HOTOVO 2026-06-29:** doménová property `cnc-nahradnidily.cz` ověřena přes DNS TXT `google-site-verification=KFvgmViiep94zi5rMQbuREHgVwCFNN48joINTuE5PAY` (TXT zůstává v DNS natrvalo). Submitnut `sitemap.xml` (48 URL). Homepage poslána přes URL Inspection → „Request indexing". POZN.: před GSC Google držel jen STARÝ Webnode cache (`www`, titulek o Fanuc/Siemens/15 letech) — nový web nebyl zaindexován vůbec. Indexace nového obsahu = dny–týdny. ZBÝVÁ (uživatel): proklikat Request indexing i pro co-dodavame/ukazky-dilu/jak-poptat/kontakty (limit ~10/den); sledovat v GSC „Stránky" postup indexace.
- [x] **On-page SEO homepage — HOTOVO 2026-06-29 (commit efb07cf, nasazeno):** H1 změněn ze sloganu na cílovou frázi **„Náhradní díly pro CNC stroje"** (nejsilnější on-page signál); slogan „Stejné díly, příjemnější cena." přesunut pod H1 jako `.hero-slogan` (teplý akcent zachován), duplicitní pretitle odebrán. Title/description/podnadpis frázi obsahovaly už předtím.
